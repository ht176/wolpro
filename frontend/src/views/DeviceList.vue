<template>
  <el-container class="h-screen w-full bg-gray-50 dark:bg-gray-900 transition-colors duration-300">
    <!-- Sidebar -->
    <el-aside width="240px" class="bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 flex flex-col transition-colors duration-300">
    ...
    <el-container>
      <!-- Header -->
      <el-header class="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 h-16 flex items-center justify-between px-6 transition-colors duration-300">
            ...
            >
              <div>
                ...
                <div class="text-2xl font-bold" :class="stat.valueClass || 'text-gray-800 dark:text-gray-100'">
                  <span v-if="stat.link" class="text-base font-medium text-blue-600 dark:text-blue-400">{{ stat.value }}</span>
              ...
              <div class="flex justify-between items-center">
                <div class="flex items-center gap-2">
                  ...
                  <el-button size="small" :loading="isCheckingStatus" @click="handleCheckStatus" type="primary" plain>
                    <template #icon><div class="i-ep-refresh w-4 h-4" /></template>
                    刷新状态
                  </el-button>
                </div>
                <el-button type="primary" @click="openAddModal">
                  <template #icon><div class="i-ep-plus w-4 h-4" /></template>
                  添加设备
                </el-button>
              </div>
            </template>
                  ...
                  <div class="flex justify-end gap-2">
                    <el-tooltip content="发送唤醒包 (WOL)" placement="top">
                      <el-button size="small" type="primary" plain @click="handleWake(scope.row)">
                        <template #icon><div class="i-ep-video-play w-4 h-4" /></template>
                      </el-button>
                    </el-tooltip>
                    <el-button size="small" plain @click="openEditModal(scope.row)">
                      <template #icon><div class="i-ep-edit w-4 h-4" /></template>
                    </el-button>
                    <el-button size="small" type="danger" plain @click="handleDelete(scope.row)">
                      <template #icon><div class="i-ep-delete w-4 h-4" /></template>
                    </el-button>
                  </div>
                </template>
              </el-table-column>
        ...
        <div v-else-if="activeTab === 'scanner'" class="animate-fade-in">
          <el-card shadow="never" class="!border-none !rounded-lg shadow-sm dark:bg-gray-800">
            <div class="flex flex-col items-center justify-center py-10" v-if="!store.isScanning && store.scannedDevices.length === 0">
              ...
              <el-button type="primary" size="large" @click="handleScan" class="px-8">
                开始扫描
              </el-button>
            </div>
